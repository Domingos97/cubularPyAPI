...
from app.models.models import SupportedLanguage, PromptTranslation, User
...
class PromptTranslationService:
    """Service for managing prompt translations"""
    ...
    async def get_prompt_translations(
        ...
        limit: int = 100,
        offset: int = 0
    ) -> List[PromptTranslation]:
        """Get prompt translations with optional filtering"""
        try:
            query = db.query(PromptTranslation)
            
            if prompt_key:
                query = query.filter(PromptTranslation.prompt_key == prompt_key)
            
            if language_code:
                query = query.filter(PromptTranslation.language_code == language_code.lower())
    ...
    @staticmethod
    async def get_prompt_translation(
        db: Session,
        prompt_key: str,
        language_code: str
    ) -> Optional[PromptTranslation]:
        """Get specific prompt translation"""
        try:
            return db.query(PromptTranslation).filter(
                and_(
                    PromptTranslation.prompt_key == prompt_key,
                    PromptTranslation.language_code == language_code.lower()
                )
            ).first()
        except Exception as e:
            logger.error(f"Error getting prompt translation: {str(e)}")
            return None
    ...
    async def create_prompt_translation(
        ...
        try:
            ...
            language = await LanguageService.get_language_by_code(db, translation_data.language_code)
            if not language:
                raise ValueError(f"Language '{translation_data.language_code}' not found")
            
            # Check if translation already exists
            existing = await PromptTranslationService.get_prompt_translation(
                db, translation_data.prompt_key, translation_data.language_code
            )
            if existing:
                raise ValueError(f"Translation already exists for key '{translation_data.prompt_key}' in language '{translation_data.language_code}'")
            
            translation = PromptTranslation(
                prompt_key=translation_data.prompt_key,
            ...
            logger.info(f"Created prompt translation: {translation.prompt_key} ({translation.language_code})")
            return translation
            
        except IntegrityError as e:
            db.rollback()
            logger.error(f"Integrity error creating prompt translation: {str(e)}")
            raise ValueError("Prompt translation key and language combination must be unique")
        except Exception as e:
            db.rollback()
    ...
    async def update_prompt_translation(
        ...
        try:
            translation = await PromptTranslationService.get_prompt_translation(
            ...
            logger.info(f"Updated prompt translation: {prompt_key} ({language_code})")
            return translation
    ...
    async def delete_prompt_translation(
        ...
        try:
            ...
            logger.info(f"Deleted prompt translation: {prompt_key} ({language_code})")
            return True
    ...
    async def get_translations_by_language(
        ...
        try:
            translations = db.query(PromptTranslation).filter(
                PromptTranslation.language_code == language_code.lower()
            ).all()
            
            return {t.prompt_key: t.translated_text for t in translations}
